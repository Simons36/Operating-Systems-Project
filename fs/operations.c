...
int flag_destroy_all;
int open_files_counter;
...
int tfs_init() {
    ...
    if(pthread_mutex_init(&mutex_cond, NULL) != 0) return -1;

    if(pthread_cond_init(&cond, NULL) != 0) return -1;
    /* create root inode */
...
int tfs_destroy() {
    ...
    if(pthread_mutex_destroy(&mutex_cond) != 0) return -1;
